		...
		<table class="table table-sm">
			...
			<tbody>
					...
					<tr>
						<td>${gbbsdto.num }</td>
						 <td>
						<a href="/gboard/view?num=${gbbsdto.num }&${pc.makeSearchURL(pc.recordCriteria.reqPage)} " class="text-decoration-none text-reset text-danger" >${gbbsdto.title }</a>
						</td>
						<td>${gbbsdto.id }</td>
						<td>${gbbsdto.cdate }</td>
						<td>${gbbsdto.hit }</td>
				...
				<form action="/gboard/list" method="post">
					<div class="row">
						<label class="col-sm-1 col-form-label col-form-label-sm px-0 mx-1"
							for="searchType"><i class="fas fa-search fa-lg pt-1 pl-1"></i></label> <select
							class="col-sm-4 custon-select custom-select-sm px-1 mr-1"
